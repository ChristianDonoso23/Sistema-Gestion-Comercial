...
class ClienteController
{
        ...
        try {
            switch ($method) {
                ...
                case 'PUT':
                    ...
                    $existingCliente = $this->clienteRepository->findById((int)$payload['id']);
                    if (!$existingCliente) {
                    ...
                    $cliente = $this->updateClienteFromPayload($existingCliente, $payload);

                    $success = $this->clienteRepository->update($cliente);
                    echo json_encode(['success' => $success, 'id' => $cliente->getId()]);
                    break;

                case 'DELETE':
                    // Por convención, el ID puede venir por query string o JSON
                    $id = null;
                    if (isset($_GET['id'])) {
                        $id = (int)$_GET['id'];
                    } else {
                    ...
                    $success = $this->clienteRepository->delete($id);
    ...
    private function updateClienteFromPayload(Cliente $cliente, array $payload): Cliente
    {
        ...
        // Actualiza según tipo específico
        if ($cliente instanceof PersonaNatural) {
